...
public class EngineCancellationTokenTests : TestBase
{
	...
	public async Task When_Cancel_Engine_Token_With_DependsOn_Then_Modules_Cancel()
	{
		...
		{
			_ = await Assert.That(host.ExecutePipelineAsync).ThrowsException();
			_ = await Assert.That(module1.Status).IsEqualTo(Status.PipelineTerminated);
		}
	}
	...
	public async Task When_Cancel_Engine_Token_Without_DependsOn_Then_Modules_Cancel()
	{
		...
		using (Assert.Multiple())
		{
			_ = await Assert.That(async () => await pipelineTask).ThrowsException();
			_ = await Assert.That(longRunningModule.Status).IsEqualTo(Status.PipelineTerminated);
			_ = await Assert.That(longRunningModule.Duration).IsLessThan(TimeSpan.FromSeconds(30));
		}
	}
