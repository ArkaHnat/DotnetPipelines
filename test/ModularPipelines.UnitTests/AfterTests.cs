...
public class AfterTests : TestBase
{
	...
	public async Task No_Exception_Thrown_When_Dependent_Module_Present()
	{
		...
		_ = await Assert.That(pipelineSummary.Status)
		...
		var module1EndTime = module1.EndTime.ToString(formatString);
		var module2StartTime = module2.StartTime.ToString(formatString);
		_ = await Assert.That(module1EndTime).IsLessThanOrEqualTo(module2StartTime);
	}
	...
	{
		_ = await Assert.That(async () => await TestPipelineHostBuilder.Create()
			.AddModule<Module4>()
			.ExecutePipelineAsync())
